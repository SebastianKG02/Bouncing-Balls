...
EndlessLoseScene::EndlessLoseScene(int id, std::string name) {
	this->sceneID = id;
	this->name = name;
}

void EndlessLoseScene::init() {
	...
	text.push_back(new sf::Text(std::string("Back to Endless Menu"), *AssetManager::getFont("title"), 30));
	...
	//Level notifier
	text.push_back(new sf::Text(std::string("Endless Level " + std::to_string(Player::getData()->endless_level+1) + " Failed."), *AssetManager::getFont("title"), 56));
...
void EndlessLoseScene::update(sf::RenderWindow* w) {
...
void EndlessLoseScene::input(sf::Event* e) {
