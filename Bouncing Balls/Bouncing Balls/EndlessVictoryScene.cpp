...
EndlessVictoryScene::EndlessVictoryScene(int id, std::string name) {
	this->sceneID = id;
	this->name = name;
}

void EndlessVictoryScene::init() {
	...
	text.push_back(new sf::Text(std::string("Back to Endless"), *AssetManager::getFont("title"), 30));
	...
	//Level notifier
	text.push_back(new sf::Text(std::string("Endless Level " + std::to_string(Player::getData()->endless_level-1) + " Complete!"), *AssetManager::getFont("title"), 56));
	...
	sprites.push_back(new sf::Sprite(*AssetManager::getTexture(std::to_string(Player::getData()->endless_last_stars) + "_stars")));
...
void EndlessVictoryScene::update(sf::RenderWindow* w) {
...
void EndlessVictoryScene::input(sf::Event* e) {
