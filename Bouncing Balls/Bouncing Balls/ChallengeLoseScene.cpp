...
ChallengeLoseScene::ChallengeLoseScene(int id, std::string name) {
	this->sceneID = id;
	this->name = name;
}

void ChallengeLoseScene::init() {
	...
	text.push_back(new sf::Text(std::string("Back to Challenge Menu"), *AssetManager::getFont("title"), 30));
	...
	//Level notifier
	text.push_back(new sf::Text(std::string("Challenge Level " + std::to_string(Player::getData()->challenge_level+1) + " Failed."), *AssetManager::getFont("title"), 56));
...
void ChallengeLoseScene::update(sf::RenderWindow* w) {
...
void ChallengeLoseScene::input(sf::Event* e) {
