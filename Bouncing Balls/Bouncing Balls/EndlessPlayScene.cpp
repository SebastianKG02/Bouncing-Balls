...
//Default constructor
EndlessPlayScene::EndlessPlayScene(int id, std::string name) {
	this->name = name;
	this->sceneID = id;
}

void EndlessPlayScene::init() {
	...
	this->rules->level = Player::getData()->endless_level;
	this->rules->rows = 6 + (rules->level * GM_ENDLESS_ROW_MOD_LVL);
	...
	this->rules->time_l2 = rules->time_l2 + (rules->time_l2 * (rules->level * GM_LEVEL_DIFF_MOD_L2));
	this->rules->time_l3 = rules->time_l3 + (rules->time_l3 * (rules->level * GM_LEVEL_DIFF_MOD_L3));
...
void EndlessPlayScene::update(sf::RenderWindow* w) {
	...
	if (status[0] == false && status[1] == false) {
		if (started == false) {
			...
			if (paused == true) {
															...
															if (map.getBall(x, y)->col == *ball->getColour()) {
																...
																if (kill_list.size() > 2) {
																	for (auto& kpos : kill_list) {
																		...
																		playerScore += (1 + (rules->level * GM_ENDLESS_PTS_MOD_LVL)) * pts_modifier * GM_PTS_PER_BALL;
																		sounds[0]->play();
																		num_balls--;
																		delete kpos;
																	}
				...
				map.update(0, (GM_BALL_MOVE_SPEED* TICK_SPEED.asMilliseconds())* (1 + (GM_ENDLESS_SPD_MOD_LVL * rules->level)));
				//Update pneumatic press locatuib
				ball_eraser.move(0, (GM_BALL_MOVE_SPEED* TICK_SPEED.asMilliseconds())* (1 + (GM_ENDLESS_SPD_MOD_LVL * rules->level)));
	...
	if (status[0] == true && status[1] == false) {
		...
		playerScore += pts_modifier * (rules->level * (1 + GM_ENDLESS_PTS_MOD_LVL)) + GM_PTS_PER_WIN;

		//Reset stars gained for finishing level
		Player::getData()->endless_last_stars = 0;
		//Reward player with points based on objective completion
		if (rules->obj_l1) {
			Player::getData()->endless_last_stars++;
			playerScore += (rules->level * (1 + GM_ENDLESS_PTS_MOD_LVL)) * GM_ENDLESS_STAR_PTS;
		}
		...
		Player::save();
		Player::getData()->endless_level++;
		
		if (playerScore > Player::getData()->endless_hs) {
			Player::getData()->endless_hs = playerScore;
		}

		SceneManager::setNext(14);
		SceneManager::next();
	}
...
void EndlessPlayScene::input(sf::Event* e) {
	p1Cannon.input(e);
}

BallColour EndlessPlayScene::getNextColour() {
	srand(time(NULL));
...
void EndlessPlayScene::updatePauseLabel() {
	if (paused == true) {
...
void EndlessPlayScene::updateScoreLabel() {
...
void EndlessPlayScene::updateClockLabel() {
...
void EndlessPlayScene::updateBallsLeft() {
...
//Add row
void EndlessPlayScene::addRowReassignSprites() {
	map.addRow(true);
