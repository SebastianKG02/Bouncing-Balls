...
CampaignInfoScene::CampaignInfoScene(int id, std::string name) {
	this->sceneID = id;
	this->name = name;
}

void CampaignInfoScene::init() {
	...
	//Title text
	text.push_back(new sf::Text(std::string("Campaign"), *AssetManager::getFont("title"), 75));
	//Set real position
	...
	text.push_back(new sf::Text(std::string("The Classic Campaign of 'Bouncing Balls',"), *AssetManager::getFont("title"), 34));
	...
	text.push_back(new sf::Text(std::string("with 6 progressively more difficult levels."), *AssetManager::getFont("title"), 34));
	...
	text.push_back(new sf::Text(std::string("Each level will have one more ball colour to deal with,"), *AssetManager::getFont("title"), 34));
	...
	text.push_back(new sf::Text(std::string("starting with two colours."), *AssetManager::getFont("title"), 34));
	...
	text.push_back(new sf::Text(std::string("The faster you complete each level, the more stars you will get."), *AssetManager::getFont("title"), 34));
...
void CampaignInfoScene::update(sf::RenderWindow* w) {
...
void CampaignInfoScene::input(sf::Event* e) {
