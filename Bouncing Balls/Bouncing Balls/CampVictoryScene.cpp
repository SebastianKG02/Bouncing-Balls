...
CampVictoryScene::CampVictoryScene(int id, std::string name) {
	this->sceneID = id;
	this->name = name;
}

void CampVictoryScene::init() {
	...
	//Level notifier
	text.push_back(new sf::Text(std::string("Level " + std::to_string(Player::getData()->last_level + 1) + " Complete!"), *AssetManager::getFont("title"), 56));
	...
	sprites.push_back(new sf::Sprite(*AssetManager::getTexture(std::to_string(Player::getData()->campaign_stars[Player::getData()->last_level]) + "_stars")));
...
void CampVictoryScene::update(sf::RenderWindow* w) {
...
void CampVictoryScene::input(sf::Event* e) {
