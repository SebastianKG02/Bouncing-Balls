...
ChallengePreScene::ChallengePreScene(int id, std::string name) {
	this->sceneID = id;
	this->name = name;
}

void ChallengePreScene::init() {
	...
	//Title text
	text.push_back(new sf::Text(std::string("Challenge Menu"), *AssetManager::getFont("title"), 75));
	//Set real position
	...
	//Current Challenge level label
	text.push_back(new sf::Text(std::string("Current Challenge Level: " + std::to_string(Player::getData()->endless_level)), *AssetManager::getFont("title"), 40));
	...
	//Current Challenge level label
	text.push_back(new sf::Text(std::string("Next Challenge Level: " + std::to_string(Player::getData()->endless_level + 1)), *AssetManager::getFont("title"), 40));
	...
	//All-time Challenge high-score label
	...
	//Current Challenge level modifiers
	text.push_back(new sf::Text(std::string("Points: +" + std::to_string(GM_CHALLENGE_PTS_MOD_LVL * Player::getData()->challenge_level) + "%, Game speed: +" + std::to_string(GM_CHALLENGE_SPD_MOD_LVL * Player::getData()->challenge_level) + "%"), *AssetManager::getFont("title"), 40));
	...
	//Next Challenge level modifiers
	text.push_back(new sf::Text(std::string("Points: +" + std::to_string(GM_CHALLENGE_PTS_MOD_LVL * (Player::getData()->challenge_level + 1)) + "%, Game speed: +" + std::to_string(GM_CHALLENGE_SPD_MOD_LVL * (Player::getData()->challenge_level + 1)) + "%"), *AssetManager::getFont("title"), 40));
...
void ChallengePreScene::update(sf::RenderWindow* w) {
	...
	if (*ui[0]->getState() == UIState::CLICK) {
		...
		ui[0]->lock();
		this->lock();
		//Go back to Challenge info scene
	...
	if (*ui[1]->getState() == UIState::CLICK) {
		...
		ui[1]->lock();
		this->lock();
		//Go back to Challenge menu
		SceneManager::setNext(17);
		SceneManager::next();
	}
...
void ChallengePreScene::input(sf::Event* e) {
